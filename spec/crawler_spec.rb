  ...
  describe Crawler do
    ...
    context "#start" do
      it "should have links in list when start robolek" do
        ...
        TrataLink.should_receive(:trata_pagina).with(dominio, "#{dominio}robots.txt", "", nil, nil, nil, nil, nil)
    ...
    context "#crawl" do
      it "should return page extract" do
        ...
        pages.each { |page| TrataLink.should_receive(:trata_pagina).with(dominio, "#{dominio}robots.txt", "", nil, nil, nil, nil, nil).and_return(trata_link) }
      ...
      it "should return two page extract" do
        ...
        TrataLink.should_receive(:trata_pagina).with(pages[0].dominio, "#{dominio}robots.txt", "", nil, nil, nil, nil, nil).and_return(trata_link)
        TrataLink.should_receive(:trata_pagina).with(pages[1].dominio, "#{dominio2}robots.txt", "", nil, nil, nil, nil, nil).and_return(trata_link)
    ...
    context "#salva_links" do
      it "should call save links in db" do
        ...
        pagina.stub(links: pages[0].links_url, code: "200", :url => dominio, :base_produtos => "", :robots => "#{dominio}robots.txt", base_preco: "", base_foto: "", base_genero: "", base_nome: "", base_tamanho: "")

        db.should_receive(:links).with(1).and_return([{"url" => dominio, "robots" => "#{dominio}robots.txt"}])

        pages.each { |page| TrataLink.should_receive(:trata_pagina).with(dominio, "#{dominio}robots.txt", "", nil, nil, nil, nil, nil).and_return(pagina) }
    ...
    context "#loop_crawl" do
      it "should return links use deep 2 for crawl" do
        ...
        TrataLink.should_receive(:trata_pagina).with(pages[0].dominio, "#{dominio}robots.txt", "", nil, nil, nil, nil, nil).and_return(pagina)
      ...
      it "should return and save produto" do
        ...
        pagina.stub(links: [], :url => dominio, code: "200", :produtos => ["#{dominio}produto/teste2"], base_preco: "", base_foto: "", base_genero: "", base_nome: "",  base_tamanho: "")

        TrataLink.should_receive(:trata_pagina).with("#{dominio}", "#{dominio}robots.txt", "#{dominio}produto/", nil, nil, nil, nil, nil).and_return(pagina)

        db.should_receive(:save_produtos).with(["#{dominio}produto/teste2"], "", "", "", "", "")
        @robo.loop_crawl(:next)
      end

      it "should save in sql db produtos" do
        dominio = "http://www.teste.com/"
        ...
        produto.stub(nome: "nome", url: "#{dominio}produtos/teste1", foto: "foto", preco: "100", genero: "Feminino", tamanho: "")

        TrataProduto.should_receive(:trata_produto).with("#{dominio}produtos/teste1", nil, nil, nil, nil, nil).and_return(produto)
